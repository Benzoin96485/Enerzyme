...
**TACE** (:code:`architecture: tace`) is a native port of Xu et al. (arXiv:2509.14961; Cartesian-3j arXiv:2512.16882) under :code:`enerzyme/models/tace/`, adapted from `xvzemin/tace <https://github.com/xvzemin/tace>`_ (MIT). The Core is the ACE message/product stack only (edge update + CgtpInteraction / Cartesian ICT contraction + CgtpACE / Cartesian product); embeddings, property heads, ZBL/LES/UIE stay outside. Switch backends with Core param :code:`tensor_basis: spherical|cartesian` (default spherical). Spherical uses e3nn CGTP paths (Lmax/lmax split, BB element-aware residual, optional density scatter-norm). Cartesian vendors :code:`cartnn` from tace v0.1.0. TECE / SO2 / RRA are not included. Emits :code:`atom_feature` with :code:`feature_irreps` / :code:`dim_feature_out` for :code:`SimpleReadout`. Examples: :code:`enerzyme/config/tace_layers_example.yaml`, :code:`tace_cartesian_layers_example.yaml`. Tests: :code:`test/test_tace_core.py`, :code:`test/test_tace_spherical_ops.py`, :code:`test/test_tace_cartesian_ops.py`. Enerzymette only needs :code:`architecture: tace` plus a resolved :code:`config.yaml`.

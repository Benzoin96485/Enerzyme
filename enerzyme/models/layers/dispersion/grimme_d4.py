...
class GrimmeD4EnergyLayer(Module):
    def __init__(
        self,
    ...
    def get_e_disp(
        ...
        Dij = Dij / self.Bohr_in_R  # convert distances to Bohr
        Zi = Za[idx_i]
        Zj = Za[idx_j]
        ...
        tmp = den * 0.5 * (1.0 + torch.erf(-self.kn * (Dij - rco) / rco))
        if self.cutoff is not None:
            tmp = tmp * smooth_transition(Dij, self.cutoff, self.cuton)
        ...
        if self.cutoff is None:
            ...
            cut6tmp6 = cut6 + tmp6
            cut8tmp8 = cut8 + tmp8
            tmpc = Dij / self.cutoff - 1
            oor6 = (
                1 / (Dij ** 6 + tmp6) - 1 / cut6tmp6 + 6 * cut6 / cut6tmp6 ** 2 * tmpc
            )
            oor8 = (
                1 / (Dij ** 8 + tmp8) - 1 / cut8tmp8 + 8 * cut8 / cut8tmp8 ** 2 * tmpc
            )
            oor6 = torch.where(Dij < self.cutoff, oor6, torch.zeros_like(oor6))
            oor8 = torch.where(Dij < self.cutoff, oor8, torch.zeros_like(oor8))

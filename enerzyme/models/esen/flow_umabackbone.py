"""
UMA eSCNMDBackbone / eSCNMDMoeBackbone subclasses for flow matching: add pre-core
``atom_embedding`` (sum of ``*_embedding`` fields from GatherAtomEmbedding) into the l=0
node channels before edge_degree_embedding and interaction blocks.
...
class ESCNMDBackboneFlow(eSCNMDBackbone):
    ...
    def forward(
        ...
        return _escnmd_flow_forward(self, data_dict, atom_embedding)


class ESCNMDMoeBackboneFlow(eSCNMDMoeBackbone):
    """Same flow injection as :class:`ESCNMDBackboneFlow`, for UMA MoE checkpoints."""
...
def build_flow_backbone(model_name: str, backbone_cfg: dict):
    """Instantiate a flow-capable UMA backbone matching ``model`` in the checkpoint config.

    Parameters
    ----------
    model_name:
        Value of ``checkpoint.model_config["backbone"]["model"]`` (registry id or class path).
    backbone_cfg:
        Backbone kwargs with ``model`` already removed.
    """
    if model_name == "escnmd_backbone":
        return ESCNMDBackboneFlow(**backbone_cfg)
    if model_name in (
        "escnmd_moe_backbone",
        "fairchem.core.models.uma.escn_moe.eSCNMDMoeBackbone",
    ) or "eSCNMDMoeBackbone" in model_name:
        return ESCNMDMoeBackboneFlow(**backbone_cfg)
    raise ValueError(
        "UMAFlowWrapperQS does not have a flow backbone wrapper for "
        f"{model_name!r}. Supported: escnmd_backbone, escnmd_moe_backbone / eSCNMDMoeBackbone."
    )
